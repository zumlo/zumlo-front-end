...
export class ZResetComponent {
  ...
  constructor(
    ...
    private storage: ZStorageService
  ) {
    this.form = formCreator.getFormGroup(
      this.configs,
      confirmPasswordValidator('oldPassword', 'newPassword')
    );
  }
  ...
  resetPassword() {
    if (this.form.invalid) {
      ...
      const forgotUser = JSON.parse(
        this.storage.getSessionStorage('forgotUser')!
      );
      ...
      this.subscription.add(
        this.authentication.resetPassword(requiredData).subscribe({
          next: (res: any) => {
            if (res.success) {
